...
function pisano(mod, len=-1){
	...
	if (len < 1){
		var seq = nbonacci(256, 0, 1, 1).map(x => x.mod(mod));
		...
		for (var i=0; i<seq.length; i++){
			p.push(seq[i].toNumber());
			console.log(seq[i].toNumber(), p);
			if (p.length > 2){ 
				p = p.slice(1, 3);
		...
		// 	if (n2.isEqualTo(0) && n1.isEqualTo(1) && cur.isEqualTo(1) && inc > 2){
		// 		modPeriod = inc;
		// 		inc = 0;
		// 	}
...
// 
// @param {Int} -> output length of array
// @return {String-Array} -> array of BigNumber strings
// 
function pell(len=1){
